...
- 2026-09-24 — `/admin/users`: คอลัมน์ "ใช้งานล่าสุด" + badge ผู้เรียนที่ไม่ active เกิน N วัน (ค่าเริ่มต้น 7, ปรับได้) + ตัวกรอง — RPC `get_user_last_active()` (staff-only). ใช้ดูเพื่อตามนักเรียนเท่านั้น ไม่เอาไปกรองออกจาก pace/ค่าเฉลี่ย — PR เปิดแล้ว, ยังไม่ db push
